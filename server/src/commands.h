...
#define CHAT_PREFIX " \7[NEO ADMIN]\1 "
...
public:
	...
	{
		m_bCapturing = true;
		m_bCaptureTruncated = false;
		m_strCapturedOutput.clear();
	}

	std::string EndCapture()
	{
		m_bCapturing = false;
		if (m_bCaptureTruncated)
		{
			if (m_strCapturedOutput.size() > 460)
				m_strCapturedOutput.resize(460);
			m_strCapturedOutput.append("\n[output truncated]");
		}

		std::string output = m_strCapturedOutput;
		m_strCapturedOutput.clear();
		return output;
	}
	...
	void Log(const LoggingContext_t* pContext, const tchar* pMessage) override
	{
		...
		if (m_bCapturing && pMessage)
		{
			constexpr std::size_t kMaxCapturedBytes = 480;
			const std::size_t available =
				m_strCapturedOutput.size() < kMaxCapturedBytes
					? kMaxCapturedBytes - m_strCapturedOutput.size()
					: 0;
			const std::string_view message(pMessage);
			m_strCapturedOutput.append(message.substr(0, available));
			m_bCaptureTruncated = m_bCaptureTruncated || message.size() > available;
		}
	}
...
private:
	...
	bool m_bCaptureTruncated = false;
	std::string m_strCapturedOutput;
};
