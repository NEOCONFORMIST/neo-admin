...
#ifdef __linux__
		// Keep the guarded binary-only deployment compatible with the 2026-08-24
		// server update until the external gamedata package is refreshed.
		if (!address)
		{
			const char* fallback = nullptr;
			if (!V_strcmp(name, "CGameRules_TerminateRound"))
				fallback = "55 48 89 E5 41 57 41 56 41 55 41 54 41 89 F4 53 48 8D 35 ? ? ? ? 48 89 FB";
			else if (!V_strcmp(name, "CCSGameRules_GoToIntermission"))
				fallback = "55 31 C0 48 89 E5 41 57 41 56 41 55 41 54 41 89 F4";

			if (fallback && V_strcmp(signature, fallback))
			{
				pSignature = IDASigToUint8Array(fallback, iLength);
				if (pSignature)
				{
					address = (*module)->FindSignature(pSignature, iLength, error);
					delete[] pSignature;
					...
					if (address)
						Message("Used built-in 2026-08-24 fallback for %s\n", name);
				}
			}
		}
#endif
