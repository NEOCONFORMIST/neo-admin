...
CConVar<bool> g_cvarKnifePropPush("zr_knife_prop_push", FCVAR_NONE, "Whether knife hits strongly push movable physics props", true);
CConVar<float> g_cvarKnifePropPushScale("zr_knife_prop_push_scale", FCVAR_NONE, "Multiplier applied to the knife's native physics force", 3.0f, true, 0.0f, true, 10.0f);
CConVar<float> g_cvarKnifePropPushMinForce("zr_knife_prop_push_min_force", FCVAR_NONE, "Minimum physics force applied by a knife hit", 18000.0f, true, 0.0f, true, 100000.0f);
CConVar<float> g_cvarKnifePropPushMaxForce("zr_knife_prop_push_max_force", FCVAR_NONE, "Maximum physics force applied by a knife hit", 45000.0f, true, 0.0f, true, 100000.0f);
CConVar<float> g_cvarKnifePropPushLift("zr_knife_prop_push_lift", FCVAR_NONE, "Upward directional bias added to a knife prop push", 0.08f, true, 0.0f, true, 0.5f);
...
CConVar<bool> g_cvarRoundStartSound("zr_sounds_round_start", FCVAR_NONE, "Whether to play the classic HL2 zombie scream at round start", true);

namespace
{
constexpr const char* kZRPreferencePath =
	"addons/cs2fixes/configs/zr/neo_admin_mode.cfg";
...
constexpr ZRConfigFile kZRConfigFiles[] = {
	{"addons/cs2fixes/configs/zr/playerclass.jsonc",
	 "addons/cs2fixes/configs/zr/playerclass.jsonc.example"},
	{"addons/cs2fixes/configs/zr/weapons.cfg",
	 "addons/cs2fixes/configs/zr/weapons.cfg.example"},
	{"addons/cs2fixes/configs/zr/hitgroups.cfg",
	 "addons/cs2fixes/configs/zr/hitgroups.cfg.example"},
};

struct ZRServerCvarSnapshot
{
	const char* name;
	std::string value;
	bool valid = false;
...
std::array<ZRServerCvarSnapshot, 5> g_ZRServerCvars{{
	{"mp_give_player_c4"},
	{"mp_friendlyfire"},
...
bool g_bZRServerCvarsCaptured = false;

void ZR_CaptureServerCvars()
{
	for (ZRServerCvarSnapshot& snapshot : g_ZRServerCvars)
	{
		ConVarRefAbstract cvar(snapshot.name, true);
		if (!cvar.IsValidRef())
		{
			Warning("Zombie Survival could not find server CVar %s\n", snapshot.name);
			continue;
		}
		snapshot.value = cvar.GetString().String();
		snapshot.valid = true;
	}
	g_bZRServerCvarsCaptured = true;
}

void ZR_ApplyServerCvars()
{
	// These values are required by the game mode and are restored on disable.
...
void ZR_RestoreServerCvars()
{
	for (ZRServerCvarSnapshot& snapshot : g_ZRServerCvars)
	{
		...
		ConVarRefAbstract cvar(snapshot.name, true);
		if (cvar.IsValidRef())
			cvar.SetString(CUtlString(snapshot.value.c_str()));
		snapshot.value.clear();
		snapshot.valid = false;
	}
	g_bZRServerCvarsCaptured = false;
}

std::filesystem::path ZR_GetGamePath(const char* relativePath)
{
	return std::filesystem::path(Plat_GetGameDirectory()) / "csgo" / relativePath;
}
}

bool ZR_EnsureConfiguration(std::string& error)
{
	error.clear();
	for (const ZRConfigFile& file : kZRConfigFiles)
	{
		const std::filesystem::path activePath = ZR_GetGamePath(file.active);
		const std::filesystem::path examplePath = ZR_GetGamePath(file.example);
		std::error_code filesystemError;
		if (std::filesystem::exists(activePath, filesystemError))
		{
			if (filesystemError ||
				!std::filesystem::is_regular_file(activePath, filesystemError) ||
				std::filesystem::is_symlink(activePath, filesystemError))
			{
				error = std::string("Zombie Survival config is not a regular file: ") + file.active;
				return false;
			}
			continue;
		}
		if (filesystemError ||
			!std::filesystem::is_regular_file(examplePath, filesystemError) ||
			std::filesystem::is_symlink(examplePath, filesystemError))
		{
			error = std::string("Zombie Survival template is missing: ") + file.example;
			return false;
		}
		...
		if (filesystemError)
		{
			error = std::string("Could not activate Zombie Survival config: ") + file.active;
			return false;
		}
	}
	return true;
...
bool ZR_SaveEnabledPreference(bool enabled, std::string& error)
{
	error.clear();
	const std::filesystem::path preferencePath = ZR_GetGamePath(kZRPreferencePath);
	std::error_code filesystemError;
	std::filesystem::create_directories(
		preferencePath.parent_path(),
		filesystemError);
	if (filesystemError)
	{
		error = "Could not create the Zombie Survival configuration directory.";
		return false;
	}

	const std::filesystem::path temporaryPath =
		preferencePath.string() + ".new";
	{
		std::ofstream output(temporaryPath, std::ios::trunc);
		if (!output)
		{
			error = "Could not write the Zombie Survival preference.";
			return false;
		}
		output << "enabled " << (enabled ? 1 : 0) << '\n';
		output.flush();
		if (!output)
		{
			error = "Could not finish writing the Zombie Survival preference.";
			return false;
		}
	}
	...
	if (filesystemError)
	{
		error = "Could not save the Zombie Survival preference.";
		return false;
	}
	return true;
}

void ZR_ApplyEnabledPreference()
{
	const std::filesystem::path preferencePath = ZR_GetGamePath(kZRPreferencePath);
	std::error_code filesystemError;
	if (!std::filesystem::exists(preferencePath, filesystemError))
		return;

	std::ifstream input(preferencePath);
	std::string key;
	int value = -1;
	if (!(input >> key >> value) || key != "enabled" ||
		(value != 0 && value != 1))
	{
		Warning("Invalid Zombie Survival preference in %s\n", kZRPreferencePath);
		return;
	}

	g_cvarEnableZR.Set(value == 1);
}
...
void CZRPlayerClassManager::PrecacheModels(IEntityResourceManifest* pResourceManifest)
{
	...
	{
		if (!pair.second->bEnabled)
			continue;
		for (const auto& pModel : pair.second->vecModels)
			if (!pModel->szModelPath.empty())
...
void CZRPlayerClassManager::ApplyBaseClassVisuals(std::shared_ptr<ZRClass> pClass, CCSPlayerPawn* pPawn)
{
	...
	if (!pModelEntry || pModelEntry->szModelPath.empty())
	{
		Warning("ZR class %s has no valid player model.\n", pClass->szClassName.c_str());
		return;
	}
	Color clrRender;
...
void CZRPlayerClassManager::CreateRegenTimer(int iPlayerSlot, CHandle<CCSPlayerPawn> hPawn, float flInterval, int iAmount)
{
	...
	if (iPlayerSlot < 0 || iPlayerSlot >= MAXPLAYERS ||
		!(flInterval > 0.0f) || !std::isfinite(flInterval) || iAmount <= 0)
	{
		return;
	}
...
void ZR_OnLevelInit()
{
	...
	// The main config is queued immediately before this function. Restore the
	// persisted Windows-app mode after it executes, then apply ZR's map settings.
	CTimer::Create(0.02f, TIMERFLAG_MAP, []() {
		if (kZombieSurvivalImplemented)
			ZR_ApplyEnabledPreference();
		else
			g_cvarEnableZR.Set(false);
		switch (zr::runtime::DetermineModeCvarTransition(
			g_bZRServerCvarsCaptured,
			g_cvarEnableZR.Get()))
		{
			case zr::runtime::ModeCvarTransition::CaptureAndApply:
				ZR_CaptureServerCvars();
				[[fallthrough]];
			case zr::runtime::ModeCvarTransition::Apply:
				ZR_ApplyServerCvars();
				break;
			case zr::runtime::ModeCvarTransition::Restore:
				ZR_RestoreServerCvars();
				return -1.0f;
			case zr::runtime::ModeCvarTransition::None:
				return -1.0f;
		}
...
{
	if (g_hRespawnToggler.IsValid() && g_hRespawnToggler.Get())
		return;

	CBaseEntity* relay = CreateEntityByName("logic_relay");
	if (!relay)
	{
		Warning("Could not create the Zombie Survival respawn relay.\n");
		return;
	}
...
void ZR_OnRoundStart(IGameEvent* pEvent)
{
	...
	for (int i = 0; i < GetGlobals()->maxClients; i++)
	{
		...
		if (pPawn)
		{
			...
			if (g_cvarRoundStartSound.Get())
			{
				CSingleRecipientFilter filter(pController->GetPlayerSlot());
				pPawn->EmitSoundFilter(filter, "zr.amb.scream");
			}
		}
	}
}
...
void ZR_OnPlayerSpawn(CCSPlayerController* pController)
{
	...
	if (bInfect)
	{
		if (CBasePlayerPawn* pPawn = pController->GetPawn())
			pPawn->m_bTakesDamage(false);
...
{
	if (!pHuman || !pVictim || !szWeapon || !*szWeapon ||
		!g_pZRWeaponConfig || !g_pZRHitgroupConfig)
	{
		return;
	}
...
{
	if (!pProjectile || !pVictim || !g_pZRWeaponConfig)
		return;
...
{
	if (!pVictimController || !pVictimController->m_bPawnIsAlive())
		return;
	...
	pEvent->SetPlayer("attacker", (pAttackerController ? pAttackerController : pVictimController)->GetPlayerSlot());
...
void ZR_StripAndGiveKnife(CCSPlayerPawn* pPawn)
{
	...
	FOR_EACH_VEC(*weapons, i)
	{
		...
		if (pWeapon && pWeapon->GetWeaponVData() &&
			pWeapon->GetWeaponVData()->m_GearSlot() == GEAR_SLOT_KNIFE)
		{
...
float ZR_MoanTimer(ZEPlayerHandle hPlayer)
{
	...
	const float interval = g_cvarMoanInterval.Get();
	if (!(interval > 0.0f) || !std::isfinite(interval))
		return -1.f;

	CCSPlayerController* pController = CCSPlayerController::FromSlot(hPlayer.GetPlayerSlot());
...
void ZR_StartMoanTimer(ZEPlayer* pPlayer)
{
	if (!pPlayer)
		return;

	float delay = 0.0f;
	if (!zr::runtime::TryInitialMoanDelay(
			g_cvarMoanInterval.Get(),
	...
	const ZEPlayerHandle hPlayer = pPlayer->GetHandle();
	CTimer::Create(delay, TIMERFLAG_MAP | TIMERFLAG_ROUND,
		[hPlayer]() { return ZR_MoanTimer(hPlayer); });
}
...
void ZR_InfectShake(CCSPlayerController* pController)
{
	...
	if (!pNetMsg)
	{
		Warning("Zombie Survival could not find the Shake user message.\n");
		return;
	}
...
std::vector<SpawnPoint*> ZR_GetSpawns()
{
	...
	if (spawns.empty())
		Warning("Zombie Survival found no usable player spawns.\n");
...
void ZR_Infect(CCSPlayerController* pAttackerController, CCSPlayerController* pVictimController, bool bDontBroadcast)
{
	...
	if (pZEPlayer && !pZEPlayer->IsInfected())
	{
		...
		ZR_StartMoanTimer(pZEPlayer);
	}
}

bool ZR_InfectMotherZombie(CCSPlayerController* pVictimController, const std::vector<SpawnPoint*>& spawns)
{
	if (!pVictimController)
		return false;
	...
	if (!pZEPlayer)
		return false;
	SpawnPoint* selectedSpawn = nullptr;
	if (g_cvarInfectSpawnType.Get() == (int)EZRSpawnType::RESPAWN)
	{
		std::size_t randomindex = 0;
		if (!zr::runtime::TryRandomIndex(spawns.size(), rand(), randomindex) ||
			!spawns[randomindex])
		{
			return false;
		}
		selectedSpawn = spawns[randomindex];
	}
	...
	if (selectedSpawn)
	{
		Vector origin = selectedSpawn->GetAbsOrigin();
		QAngle rotation = selectedSpawn->GetAbsRotation();
	...
	g_MotherZombies.push_back(pZEPlayer->GetHandle());
	return true;
}
...
void ZR_InitialInfection()
{
	...
	for (int i = 0; i < GetGlobals()->maxClients; i++)
	{
		CCSPlayerController* pController = CCSPlayerController::FromSlot(i);
		if (!pController || !pController->IsConnected() ||
			pController->m_iTeamNum() != CS_TEAM_CT || !pController->GetZEPlayer())
			continue;
	...
	// Clamp the requested count to the number of valid candidates. A bad
	// minimum-count setting must never make this loop select one player twice.
	int iMZToInfect = zr::runtime::MotherZombieCount(
		vecCandidateControllers.size(),
		g_cvarInfectSpawnMZRatio.Get(),
		g_cvarInfectSpawnMinCount.Get(),
		g_cvarInfectSpawnMinCountReq.Get());
	bool vecIsMZ[MAXPLAYERS] = {false};

	// get spawn points
	std::vector<SpawnPoint*> spawns;
	if (iMZToInfect > 0 &&
		g_cvarInfectSpawnType.Get() == (int)EZRSpawnType::RESPAWN)
	{
		spawns = ZR_GetSpawns();
	}
	if (iMZToInfect > 0 &&
	...
	while (iMZToInfect > 0)
	{
		...
		if (iFailSafeCounter >= 5)
		{
			for (int i = 0; i < vecCandidateControllers.size(); i++)
			{
				...
				if (!pPlayer)
					continue;
				const int slot = pPlayer->GetPlayerSlot().Get();
				const bool selectedThisRound =
					slot >= 0 && slot < MAXPLAYERS && vecIsMZ[slot];
				if (pPlayer->GetImmunity() < 100 ||
					(iFailSafeCounter >= 6 && !selectedThisRound))
		...
		{
			std::size_t randomindex = 0;
			if (!zr::runtime::TryRandomIndex(
					vecSurvivorControllers.size(), rand(), randomindex))
			{
				break;
			}

			CCSPlayerController* pController = vecSurvivorControllers[randomindex];
			...
			if (!ZR_InfectMotherZombie(pController, spawns))
			{
				vecSurvivorControllers.erase(vecSurvivorControllers.begin() + randomindex);
				continue;
			}
	...
	for (int i = 0; i < GetGlobals()->maxClients; i++)
	{
		...
		pPlayer->SetImmunity(zr::runtime::ReduceImmunity(
			pPlayer->GetImmunity(), g_cvarMZImmunityReduction.Get()));
	}
...
void ZR_StartInitialCountdown()
{
	...
	const std::uint64_t minimum = static_cast<std::uint64_t>(
		g_cvarInfectSpawnTimeMin.Get());
	const std::uint64_t maximum = static_cast<std::uint64_t>(
		g_cvarInfectSpawnTimeMax.Get());
	const std::uint64_t span = maximum - minimum + 1;
	const int countdown = static_cast<int>(minimum +
		(static_cast<std::uint64_t>(rand()) % span));
	auto iSecondsElapsed = std::make_shared<int>(0);
	CTimer::Create(0.0f, TIMERFLAG_MAP | TIMERFLAG_ROUND, [countdown, iSecondsElapsed]() {
		if (g_ZRRoundState != EZRRoundState::ROUND_START)
			return -1.0f;

		const int g_iInfectionCountDown = countdown - *iSecondsElapsed;
...
void ZR_TryKnifePushProp(CBaseEntity* pProp, CTakeDamageInfo* pInfo)
{
	if (!g_cvarEnableZR.Get() || !g_cvarKnifePropPush.Get() || !pProp || !pInfo)
		return;

	const char* pszPropClass = pProp->GetClassname();
	if (!pszPropClass || !zr::prop_push::IsMovablePhysicsPropClass(pszPropClass) || pProp->m_nActualMoveType() != MOVETYPE_VPHYSICS)
		return;

	const CEntityInstance* pAbility = pInfo->m_hAbility.Get();
	const char* pszAbilityClass = pAbility ? pAbility->GetClassname() : nullptr;
	if (!(pInfo->m_bitsDamageType & DMG_SLASH) || !pszAbilityClass || !zr::prop_push::IsKnifeWeaponClass(pszAbilityClass))
		return;

	CBaseEntity* pAttacker = reinterpret_cast<CBaseEntity*>(pInfo->m_hAttacker.Get());
	if (!pAttacker || !pAttacker->IsPawn())
		return;

	CCSPlayerPawn* pAttackerPawn = reinterpret_cast<CCSPlayerPawn*>(pAttacker);
	Vector vecPushDirection = pInfo->m_vecDamageForce;
	const float flNativeForce = sqrtf(vecPushDirection.x * vecPushDirection.x + vecPushDirection.y * vecPushDirection.y + vecPushDirection.z * vecPushDirection.z);
	if (flNativeForce <= 0.001f)
		AngleVectors(pAttackerPawn->m_angEyeAngles(), &vecPushDirection);
	else
		vecPushDirection *= 1.0f / flNativeForce;

	vecPushDirection.z += g_cvarKnifePropPushLift.Get();
	if (VectorNormalize(vecPushDirection) <= 0.0f)
		return;

	const float flForce = zr::prop_push::ComputeForceMagnitude(
		flNativeForce,
		g_cvarKnifePropPushScale.Get(),
		g_cvarKnifePropPushMinForce.Get(),
		g_cvarKnifePropPushMaxForce.Get());
	pInfo->m_vecDamageForce = vecPushDirection * flForce;

	const float flPositionLengthSquared = pInfo->m_vecDamagePosition.x * pInfo->m_vecDamagePosition.x
		+ pInfo->m_vecDamagePosition.y * pInfo->m_vecDamagePosition.y
		+ pInfo->m_vecDamagePosition.z * pInfo->m_vecDamagePosition.z;
	if (flPositionLengthSquared <= 0.001f)
		pInfo->m_vecDamagePosition = pProp->GetAbsOrigin();

	pInfo->m_nDamageFlags = static_cast<TakeDamageFlags_t>(
		static_cast<uint64>(pInfo->m_nDamageFlags) & ~static_cast<uint64>(DFLAG_SUPPRESS_PHYSICS_FORCE));
}
...
bool ZR_Hook_OnTakeDamage_Alive(CTakeDamageInfo* pInfo, CCSPlayerPawn* pVictimPawn)
{
	...
	if (pVictimPawn->m_iTeamNum() == CS_TEAM_T &&
		zr::runtime::ShouldPlayOneInN(rand(), g_cvarGroanChance.Get()))
	...
	if (pAttackerPawn->m_iTeamNum() == CS_TEAM_CT && pVictimPawn->m_iTeamNum() == CS_TEAM_T)
	{
		...
		if (g_cvarNapalmGrenades.Get() && bGrenade)
		{
			...
			const float fullDamage = std::max(0.001f, g_cvarNapalmFullDamage.Get());
			float flDuration = (pInfo->m_flDamage / fullDamage) * g_cvarNapalmDuration.Get();
...
void ZR_OnPlayerTakeDamage(CCSPlayerPawn* pVictimPawn, const CTakeDamageInfo* pInfo, const float damage)
{
	...
	if (pKillerPawn->m_iTeamNum() == CS_TEAM_CT && pVictimPawn->m_iTeamNum() == CS_TEAM_T)
	{
		...
		if (flCashScale > 0)
		{
			...
			if (pKillerController && pKillerController->m_pInGameMoneyServices)
			{
