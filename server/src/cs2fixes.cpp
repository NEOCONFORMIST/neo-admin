...
bool CS2Fixes::Load(PluginId id, ISmmAPI* ismm, char* error, size_t maxlen, bool late)
{
	...
	if (g_VoiceBridge.ConfigureFromEnvironment())
		META_CONPRINTF("[VoiceBridge] Legacy fixed-destination fallback enabled.\n");
	else
		META_CONPRINTF("[VoiceBridge] Authenticated dynamic peer mode ready.\n");
	...
	SH_ADD_HOOK(IServerGameDLL, GameFrame, g_pSource2Server, SH_MEMBER(this, &CS2Fixes::Hook_GameFramePost), false);
	...
	{
		META_CONPRINTF(
			"[NEO ADMIN] UDP control could not start: %s\n",
			NeoPtt_GetLastError());
	}
	else if (NeoPtt_NeedsFirstOwnerSetup())
	{
		META_CONPRINTF("\n");
		META_CONPRINTF("[NEO ADMIN] FIRST INSTALLATION\n");
		META_CONPRINTF(
			"[NEO ADMIN] One-time setup code: %s\n",
			NeoPtt_GetFirstOwnerSetupCode());
		META_CONPRINTF(
			"[NEO ADMIN] In the Windows app, choose "
			"Settings > Initial Server Setup.\n");
		META_CONPRINTF(
			"[NEO ADMIN] This code stops working after the first "
			"Owner account is created.\n\n");
	}
	if (NeoPtt_IsListening())
	{
		META_CONPRINTF(
			"[NEO ADMIN] SQLite storage: %s\n",
			neo_admin::DatabasePath().c_str());
	}
...
bool CS2Fixes::Unload(char* error, size_t maxlen)
{
	...
	SH_REMOVE_HOOK(IServerGameDLL, GameFrame, g_pSource2Server, SH_MEMBER(this, &CS2Fixes::Hook_GameFramePost), false);
...
void CS2Fixes::Hook_DispatchConCommand(ConCommandRef cmdHandle, const CCommandContext& ctx, const CCommand& args)
{
	...
	if (iCommandPlayerSlot != -1 && !V_strcmp(args.Arg(0), "menuselect"))
	{
		auto pController = CCSPlayerController::FromSlot(iCommandPlayerSlot);
		if (NeoAdminMenu_HandleSelection(pController, V_atoi(args[1])))
	...
	{
		auto pController = CCSPlayerController::FromSlot(iCommandPlayerSlot);
		if (NeoAdminMenu_HandleChatInput(pController, args[1]) ||
			NeoAdminMenu_TryOpenChatCommand(pController, args[1]))
		{
			RETURN_META(MRES_SUPERCEDE);
		}
	}
...
void CS2Fixes::Hook_StartupServer(const GameSessionConfiguration_t& config, ISource2WorldSession* pSession, const char* pszMapName)
{
	...
	if (g_VoiceBridge.IsConfigured())
		g_VoiceBridge.SetCurrentMap(pszMapName);
...
void CS2Fixes::Hook_ClientPutInServer(CPlayerSlot slot, char const* pszName, int type, uint64 xuid)
{
	...
	if (g_VoiceBridge.IsConfigured())
		g_VoiceBridge.SendPlayerConnected(xuid, slot.Get(), pszName);
...
void CS2Fixes::Hook_ClientDisconnect(CPlayerSlot slot, ENetworkDisconnectionReason reason, const char* pszName, uint64 xuid, const char* pszNetworkID)
{
	...
	if (g_VoiceBridge.IsConfigured())
		g_VoiceBridge.SendPlayerDisconnected(xuid, slot.Get(), pszName);
	NeoAdminMenu_OnClientDisconnect(slot.Get());
...
void CS2Fixes::Hook_GameFramePost(bool simulating, bool bFirstTick, bool bLastTick)
{
    ...
    ++voiceBridgeDebugFrames;

    if (voiceBridgeDebugFrames == 1 ||
        voiceBridgeDebugFrames % 320ULL == 0)
    {
        Message(
            "[VoiceBridge] GameFrame active: simulating=%d first=%d last=%d frames=%llu\n",
            simulating,
            bFirstTick,
            bLastTick,
            static_cast<unsigned long long>(voiceBridgeDebugFrames));
    }
	/**
	 * simulating:
	 * ***********
	...
	if (simulating && g_VoiceBridge.ShouldSendPositionFrame())
	{
		for (int i = 0; i < GetGlobals()->maxClients; ++i)
		{
								  ...
								  CBitVec<16384>&, const Entity2Networkable_t** pNetworkables, const uint16* pEntityIndicies, int nEntities)
{
	...
	for (int i = 0; i < infoCount; i++)
	{
		...
		for (int j = 0; j < GetGlobals()->maxClients; j++)
		{
			...
			CPointWorldText* pAdminMenu = NeoAdminMenu_GetDisplay(j);
			if (pAdminMenu)
				pInfo->m_pTransmitEntity->Clear(pAdminMenu->entindex());
...
bool CS2Fixes::Hook_ProcessVoiceData(const CCLCMsg_VoiceData_t& msg)
{
	...
	if (g_VoiceBridge.IsConfigured() && msg.has_audio())
	{
		const CMsgVoiceAudio& audio = msg.audio();
		std::vector<std::uint32_t> packetOffsets;
		packetOffsets.reserve(audio.packet_offsets_size());
		for (int i = 0; i < audio.packet_offsets_size(); ++i)
			packetOffsets.push_back(audio.packet_offsets(i));

		std::uint64_t steamId = msg.xuid();
		if (steamId == 0)
		...
		g_VoiceBridge.SendVoice(
			...
			audio.sample_rate(),
			audio.sequence_bytes(),
			audio.section_number(),
			audio.uncompressed_sample_offset(),
			audio.num_packets(),
			packetOffsets,
			audio.voice_data(),
			audio.voice_level());
						   ...
						   bool background)
{
	...
	// Always let ZR restore its persisted mode after the queued main config has
	// run. The delayed initializer exits immediately when the mode is disabled.
	ZR_OnLevelInit();
