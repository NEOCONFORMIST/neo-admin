...
inline constexpr bool kZombieSurvivalImplemented = false;
...
struct ZRModelEntry
{
	...
	{
		std::size_t index = 0;
		return zr::runtime::TryRandomIndex(vecSkins.size(), rand(), index)
			? vecSkins[index] : 0;
	}
};
...
struct ZRClass
{
	...
	{
		std::size_t index = 0;
		return zr::runtime::TryRandomIndex(vecModels.size(), rand(), index)
			? vecModels[index] : nullptr;
	};
};
