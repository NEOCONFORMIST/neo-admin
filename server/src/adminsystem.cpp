...
bool ExecuteServerConsoleCommand(
	...
	std::string& output)
{
	...
	// Register only when the first console command is run. Registering this
	// listener during plugin startup can make Source 2 log_flags unstable.
	ExecuteOnce(
		LoggingSystem_RegisterBackdoorLoggingListener(&g_LoggingListener);
		LoggingSystem_EnableBackdoorLoggingListeners(true););

	CCommand args;
	args.Tokenize(commandLine);
	if (args.ArgC() < 1)
		return false;

	g_LoggingListener.BeginCapture();

	ConCommandRef cmdRef(args[0], true);
	if (cmdRef.IsValidRef())
	{
		CCommandContext context(CT_FIRST_SPLITSCREEN_CLIENT, -1);
		cmdRef.Dispatch(context, args);
		output = g_LoggingListener.EndCapture();
		return true;
	}

	ConVarRefAbstract cvarRef(args[0], true);
	if (cvarRef.IsValidRef())
	{
		if (args.ArgC() == 1)
			ConVar_PrintDescription(&cvarRef);
		else
			cvarRef.SetString(args.ArgS());

		output = g_LoggingListener.EndCapture();
		return true;
	}

	(void)g_LoggingListener.EndCapture();
	g_pEngineServer2->ServerCommand(commandLine);
	output = "Command queued by the CS2 server.";
	return true;
}
...
CON_COMMAND_CHAT_FLAGS(rcon, "<command> - Send a command to server console", ADMFLAG_RCON)
{
	...
	const bool executed = ExecuteServerConsoleCommand(args.ArgS(), output);
	...
	ClientPrint(
		...
		HUD_PRINTTALK,
		executed
			? CHAT_PREFIX "Command executed, see console for output (if any)"
			: CHAT_PREFIX "Command could not be executed");
}
